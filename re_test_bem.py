...
if __name__ == '__main__':
    ...
    base_main_resume = './net_model/_102212_imagenet/weights__base__19_2502_0.7547_0.8958.pth' 
    bem_main_resume = './net_model/_102212_imagenet/weights__base__83_2502_0.7661_0.9005.pth' 
    pdb.set_trace()
    ...
    with torch.no_grad():
        ...
        for batch_cnt_val, data_val in enumerate(dataloader):
            ...
            for train_batch_cnt, train_data in enumerate(retrain_dataloader):
                ...
                train_labels = Variable(torch.from_numpy(np.array(train_labels)).long().cuda())

                train_outputs, train_cls_feat, train_avg_feat, train_top_avg_feat = model(train_inputs, train_labels, train_img_name)
                train_sub_dist = torch.mm(avg_feat, train_avg_feat.t()) 
                dist_gather.append(train_sub_dist.detach().cpu()) 
                label_gather.extend(train_labels.tolist())
            ...
            #for feat, sub_name in zip(avg_feat.detach().cpu(), img_name): 
            #    save_path = os.path.join(save_base_dir, sub_name.split('/')[-1] + '.pt') 
            ...
            #for sub_pred, sub_gt, sub_name in zip(top3_pos[:, 0].tolist(), labels.tolist(), img_name): 
            #    gt_count_tensor[sub_gt] += 1 
            #    if sub_pred == sub_gt: 
            #        pred_count_tensor[sub_pred] += 1
            #        correct_name_gather.append(sub_name)
